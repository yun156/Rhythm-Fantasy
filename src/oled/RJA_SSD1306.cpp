...
{
    // 1) 어드레싱 범위 고정 (수평모드, 전체 컬럼/페이지)
    uint8_t cmds[] = {
        0x00, 0x20, 0x00,      // Memory mode: Horizontal
        0x00, 0x21, 0x00, 0x7F,// Column 0..127
        0x00, 0x22, 0x00, 0x07 // Page   0..7
    };
    HAL_I2C_Master_Transmit(hi2cI, CAddress, cmds, sizeof(cmds), 10);

    // 2) 16바이트씩 패킹해서 쓰기
    uint8_t pkt[1 + 16];
    pkt[0] = 0x40; // data marker
		...
		for (int8_t j = 0; j<8; j++)
		{
			if (line & 0x1)
			{
				...
				{  // big size
					int x0 = x + i*size;
					int y0 = y + j*size;
					fillRect(x0, y0, x0 + size - 1, y0 + size - 1, colour);
			...
			else if (bg != colour)
			{
				if (size == 1) // default size
				{
					pixel(x+i, y+j, bg, 1);	//frame[x+i][y+j] = bg;
				}
				else
				{  // big size
					int x0 = x + i*size;
					int y0 = y + j*size;
					fillRect(x0, y0, x0 + size - 1, y0 + size - 1, bg);
...
void OLED::blitXBM(int x, int y, int w, int h,
                   const uint8_t* bits,
                   bool colour, bool locked, bool lsb_first)
{
    // XBM은 보통 LSB-first 비트 오더를 사용.
    const int stride = (w + 7) >> 3; // 한 줄당 바이트 수

    for (int yy = 0; yy < h; ++yy) {
        for (int xx = 0; xx < w; ++xx) {
            const int byte_index = yy * stride + (xx >> 3);
            // pgm_read_byte는 이미 헤더에 매크로로 정의되어 있음
            const uint8_t b = pgm_read_byte(bits + byte_index);

            int bit_index = (xx & 7);
            if (!lsb_first) bit_index = 7 - bit_index;

            // XBM에서 해당 비트가 1이면 픽셀 ON으로 취급
            if (b & (1u << bit_index)) {
                pixel(x + xx, y + yy, colour, locked);
            } else {
                // 배경을 항상 지우고 싶으면 다음 한 줄을 주석 해제
                // pixel(x + xx, y + yy, 0, locked);
            }
        }
    }
}
...
void OLED::setOrientation(bool flip_h, bool flip_v)
{
    ...
    // 세그먼트 리맵: 좌우 반전
    b[1] = flip_h ? 0xA1 : 0xA0;
    HAL_I2C_Master_Transmit(hi2cI, CAddress, b, 2, 10);

    // COM 스캔 방향: 상하 반전
    b[1] = flip_v ? 0xC8 : 0xC0;
    HAL_I2C_Master_Transmit(hi2cI, CAddress, b, 2, 10);
}
...
{
    uint8_t b[2] = {0x00, 0xD3};
    HAL_I2C_Master_Transmit(hi2cI, CAddress, b, 2, 10);
    b[1] = (uint8_t)(ofs & 0x3F);
    HAL_I2C_Master_Transmit(hi2cI, CAddress, b, 2, 10);
}

void OLED::setStartLine(uint8_t line)
{
    uint8_t b[2] = {0x00, (uint8_t)(0x40 | (line & 0x3F))};
    HAL_I2C_Master_Transmit(hi2cI, CAddress, b, 2, 10);
}

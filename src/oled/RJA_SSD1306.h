...
//Includes
#include "hal_linux_i2c_shim.h"  //Change this based on your STM32 Series
#include <math.h>
#include <stdio.h>
#include <string>
...
public:
	...
	// XBM(1bpp) 비트맵을 (x,y)에 w×h로 그리기. 기본은 LSB-first.
void blitXBM(int x, int y, int w, int h,
             const uint8_t* bits,
             bool colour, bool locked = 1, bool lsb_first = true);
        void setOrientation(bool flip_h, bool flip_v);
        void setDisplayOffset(uint8_t ofs);   // 0..63
	void setStartLine(uint8_t line);      // 0..63	
